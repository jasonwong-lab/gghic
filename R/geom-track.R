...
#' tracks below Hi-C heatmaps. Data is read from standard genomic file formats
#' (BigWig, BED, bedGraph) or provided as GRanges objects. Each track shows signal
#' intensity as a filled area plot with automatic y-axis scaling and labeled axes.
#' Multiple tracks can be stacked vertically with independent or shared scaling.
#'
#' @inheritParams ggplot2::geom_polygon
#' @inheritParams geom_hic
#' @param data_paths Character vector of file paths to genomic signal files. Supported
#'   formats include BigWig (.bw, .bigWig), BED, and bedGraph files. Files are
#'   automatically read using rtracklayer. Named vectors are recommended to provide
#'   track labels (e.g., `c("H3K27ac" = "chip.bw", "ATAC" = "atac.bw")`).
#'   Either `data_paths` or `track_grs` must be provided. Default is `NULL`.
#' @param track_grs Named list of GRanges objects containing genomic signal data.
#'   Each GRanges should have a `score` metadata column with signal values. Names
#'   are used as track labels. Either `data_paths` or `track_grs` must be provided.
#'   Default is `NULL`.
...
#' @param fill Character string or named character vector specifying fill colors
#'   for tracks. If named, names should match track names. If unnamed, colors are
#'   recycled across tracks. Default is `"black"`.
#' @param fontsize Numeric value specifying the font size for track labels and
#'   axis annotations. Default is `5`.
...
#'
#' * **Independent scaling** (`"auto"`): Each track scaled to its own maximum,
#'   best for comparing patterns within each dataset
#' * **Shared scaling** (`"maximum"`): All tracks use the same scale, enabling
#'   direct amplitude comparison between tracks
#' * **Fixed scaling** (numeric): Manually set maximum value(s) for consistent
#'   scaling across multiple plots or known value ranges
#'
#' ## Color Specification
...
#' * **Rasterization** (`rasterize = TRUE`): Converts high-resolution tracks to
#'   raster images, dramatically reducing file size and rendering time
...
#'
#' @seealso
#' * [geom_track2()] for a simplified version with direct data frame input
