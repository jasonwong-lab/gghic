...
#'
#' @param pairs Data frame with columns: `read_name`, `chrom1`, `pos1`,
#'   `chrom2`, `pos2`. For large files, use `pairs_file` instead.
#' @param pairs_file Character. Path to pairs file (supports .gz).
#' @param cache External pointer from `readPairsCache()`. Faster for repeated
#'   operations.
#' @param return_cache Logical. Return list with bin size and cache pointer
#'   (default: FALSE).
#' @param bin_size Integer. Bin size in base pairs.
#' @param min_contacts Integer. Minimum contacts per bin (default: 1000).
...
#' @param target_coverage Numeric (0-1). Target coverage fraction (default:
#'   0.8).
#'
#' @return
