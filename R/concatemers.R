#' concatemers2gis
#'
#' @description Convert concatemer reads to pairwise interactions in a
#'   GInteractions object.
#' @param grs A GRanges object of concatemer reads.
...
#' @param bin_size The size of the bins to divide the genomic region into.
#'   Default is `100000` (100 kb).
#' @param read_group The metadata column in `grs` that defines the read groups.
#'   Default is `"read_name"`.
#' @return A [InteractionSet::GInteractions] object.
#' @examples
#' \dontrun{
#' }
#' @export concatemers2gis
...
concatemers2gis <- function(
  ...
  concatemer_interactions <- split(df_with_bins, df_with_bins[[read_group]]) |>
    purrr::map_dfr(function(x) {
      ...
      if (length(bin_ids) < 2) return(NULL)
  ...
  S4Vectors::mcols(gis) <- counts_interaction[, c("count", "bin1_id", "bin2_id")]
