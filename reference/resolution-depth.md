...
- return_cache:
  ...
  Logical, if TRUE, `findOptimalResolutionChunked()` returns a list with
  both the optimal bin size and the cache pointer. Default: FALSE.
...
- `calculateResolutionDepthChunked()`: Same as above, reads file in
  chunks.
